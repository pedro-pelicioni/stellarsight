...
The result is published in [EVIDENCE.md](EVIDENCE.md#interoperability-one-client-two-facilitators)
and regenerated into [`docs/status/interop-discovery.json`](status/interop-discovery.json).
...
MCP is served over stdio (`apps/agent/src/mcp-server.mjs`) and over Streamable HTTP on the hosted endpoint (`https://stellarsight.xyz/mcp`, `api/mcp.mjs`). The hosted endpoint operates in stateless mode, enforces per-IP rate limiting, and marks seller-supplied text with untrusted markers (`[UNTRUSTED_SELLER_CONTENT: ...]`) to close T5. `stellarsight_pay` is refused server-side on the hosted endpoint so buyer signing keys remain client-side.
...
- `npm run verify:conformance` — a stock `@x402/fetch` client (`wrapFetchWithPayment`, no
  ...
  hash.
- `npm run verify:api` — 54 checks driving the real `withBazaar()` client from
...
The full analysis is [THREAT-MODEL.md](THREAT-MODEL.md) — thirteen threats, each with the
...
| Silent spec drift | stock-client conformance vs the live stack; 54 API checks + suite nightly in CI | Caught a real bug already |
...
[EVIDENCE.md](EVIDENCE.md#dependency-licences) and regenerated into
[`docs/status/licenses.json`](status/licenses.json) on every run.
...
the conformance cadence is scheduled to continue past the grant window: CI runs the 54
API-conformance checks, the full test suite and the search-quality regression gate nightly
on Node 24, and the end-to-end settled-payment conformance (`npm run verify:conformance`)
...
| Integrity is real | `npm test` → 279 tests, 70 adversarial |
