...
3. **The one field CDP carries that this catalog does not is `quality`.** It is noted rather
   than copied: what it means is not specified anywhere we can point at, and adding a field
   whose semantics we would be guessing at is how a catalog starts lying. The ranking signal
   this catalog does publish — `settlements`, with its provenance label (§3.5) — is defined
   in §4.1. Conversely the two fields CDP adds to `accepts[]`, `currency` and `recipient`,
   duplicate `asset` and `payTo` and are not in the shipped schema.
...
| | **§5.4** — we propose implementing #3134 rather than authoring a fourth `scheme_upto_stellar.md`; if the TSC prefers that we author one, T2 absorbs it at no change in cost | §6.1 |
