...
Every number below is generated. `npm run evidence:build` rewrites
[EVIDENCE.md](EVIDENCE.md) and the README from `docs/status/*.json`; nothing in that
directory is hand-entered.
...
| 5 | A settled transaction hash published per network per scheme | **Delivered** for `exact`/testnet · `upto` is **T2** · pubnet is **T3** | [TESTNET-TXS.md](TESTNET-TXS.md) |
...
| `upto` implemented as standardized, with an interop report | **T2** | [ARCHITECTURE §6.3](ARCHITECTURE.md#63-what-tranche-2-delivers) |
