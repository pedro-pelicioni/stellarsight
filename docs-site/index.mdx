...
<CardGroup cols={2}>
  <Card title="Verify the claims in 60 seconds" icon="stopwatch" href="/quickstart">
    Every number this project publishes is checkable: settled hashes, 279 tests,
    54 conformance checks, measured search quality.
  </Card>
  ...
  <Card title="Buy from an agent" icon="robot" href="/guides/agent">
    An MCP server with four tools — search, browse, describe, pay — over stdio or
    hosted at stellarsight.xyz/mcp, with input and output schemas and a 17-code error enum.
  </Card>
...
| The wire format holds | [API contract](/api/contract) — enforced by 54 stock-client checks in CI |
