...
|---|---|
| `npm test` | 279 tests, 0 failing — 70 of them adversarial catalog-integrity cases |
| `npm run verify:api` | 54 conformance checks, the load-bearing ones driven by the stock `withBazaar()` client |
...
Node 24 against a real Redis, with a merge-blocking regression gate on
search quality.
