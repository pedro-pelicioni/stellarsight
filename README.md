...
[![settled x402 payments](https://img.shields.io/badge/settled_x402_payments-101-blue)](docs/TESTNET-TXS.md)
...
| Catalog integrity is real, not decorative | `npm test` → 279 tests, 0 failing (70 of them adversarial) |
...
| It stays that way | [CI](../../actions) runs the suite, the 54 conformance checks and the site build on Node 24, the version the deployment runs, with a real Redis so nothing skips for want of one |
...
| [`packages/express`](packages/express#readme) | Drop-in x402 paywall middleware for Express: price a route, take payment in a Stellar token, and get listed in the bazaar before the first payment. Ships a `stellarsight-seller check` CLI that validates a listing locally before it is announced. 45 of the 279 tests are its. On npm: `npm i @stellarsight/express` |
...
109 in total, and the split matters: **101 are x402 payments** and 8 are setup and cleanup
...
The 101 also split by what produced them, because a settlement count without that is not
evidence of anything: 55 from `npm run evidence:batch` (a serial script paying our own
seller, prefixed `load:`), 23 from the demo loop, 23 from stock-client conformance runs.
<!-- /evidence:tally -->
